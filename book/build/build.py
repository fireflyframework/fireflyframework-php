"""Build *LaraFly by Example* into EPUB + PDF from a book manifest.
...
localized ``labels`` (e.g. the Contents heading) and ``output_basename``.
    ...
    book/build/run.sh                       # English  -> larafly-by-example.{epub,pdf}
    book/build/run.sh --config book.es.yaml # Spanish  -> larafly-by-example-es.{epub,pdf}
"""
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Build LaraFly by Example (EPUB + PDF).")
    ap.add_argument("--config", default="book.yaml",
                    help="Manifest file under book/ (default: book.yaml).")
    ap.add_argument("--out", default=None,
                    help="Output basename (default: manifest 'output_basename' or 'larafly-by-example').")
    args = ap.parse_args(argv)

    cfg = yaml.safe_load((BOOK / args.config).read_text())
    man = BOOK / cfg.get("manuscript_dir", "src")
    contents_label = cfg.get("labels", {}).get("contents", "Contents")
    out_base = args.out or cfg.get("output_basename") or "larafly-by-example"
