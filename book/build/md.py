...
Custom block directives on top of python-markdown:
  ...
  ::: listing <file-label> | <caption>       (block; code lines until a lone ':::')
      <code...>
  :::
Plain ``` fences still work via codehilite (real chapters use ```php fences for
listings; verify_code.py lints those directly with `php -l`). Callouts use the
admonition extension (note/tip/warning + a custom 'laravel' parity callout).
"""
...
_EXT = {"php":"php", "phtml":"html+php", "blade":"html+php",
        "yaml":"yaml", "yml":"yaml", "json":"json", "env":"ini", "ini":"ini",
...
class _Directives(Preprocessor):
    ...
    def _listing(self, file_label: str, cap: str | None, code: str) -> str:
        lang = _EXT.get(file_label.rsplit(".", 1)[-1].lower(), "php") if "." in file_label else "php"
        try:
...
_ADM_ICON = {
    ...
    "laravel": '<svg class="adm-ico" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" aria-hidden="true">'
               '<path d="M10 2.2l6.4 3.7v8.2L10 17.8l-6.4-3.7V5.9z" fill="none" stroke="#ff2d20" '
               'stroke-width="1.5" stroke-linejoin="round"/>'
               '<path d="M10 6.4v7.2M6.6 8.3l3.4-1.9 3.4 1.9" stroke="#ff2d20" stroke-width="1.3" '
               'fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>',
}

class _AdmonitionIcons(Postprocessor):
    """Inject an inline SVG icon at the start of each callout title."""
    _RE = re.compile(r'(<div class="admonition (?P<t>note|tip|warning|laravel)">\s*<p class="admonition-title">)')
    def run(self, text: str) -> str:
        return self._RE.sub(lambda m: m.group(1) + _ADM_ICON.get(m.group("t"), ""), text)

class LaraflyExtension(Extension):
    def __init__(self, base: Path, **kw):
        self.base = base
        super().__init__(**kw)
    def extendMarkdown(self, md):
        md.preprocessors.register(_Directives(md, self.base), "larafly_directives", 28)
        md.postprocessors.register(_AdmonitionIcons(md), "larafly_adm_icons", 5)
