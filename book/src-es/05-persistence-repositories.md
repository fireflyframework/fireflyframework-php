...
Al terminar este capítulo conocerás los dos puertos que implementa todo repositorio de LaraFly, cómo `EloquentRepository` te da CRUD completo por el precio de una sola asignación a `$model`, cómo el nombre de un método de consulta derivada se compila en una consulta Eloquent real sin cuerpo propio, el escape `#[Query]` para cualquier cosa que un nombre no pueda expresar con limpieza, cómo las `Specification` componen predicados reutilizables, cómo la **consulta por ejemplo** convierte una entidad a medio rellenar en una consulta sin escribir un solo predicado, qué dice de un método cada uno de los cuatro atributos de método (`#[Modifying]`, `#[Projection]`, `#[Lock]`, `#[EntityGraph]`), los objetos de valor `Page`/`Pageable`/`Sort` que transportan la paginación de principio a fin y el `Slice` que deliberadamente no cuenta, la familia `DataAccessException` a la que se traduce todo error de driver — y dónde esa traducción deliberadamente no ocurre — y el puente de eventos de dominio que conecta la llamada `save()` de un repositorio con el agregado del Capítulo 6.
