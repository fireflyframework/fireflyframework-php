...
1. **Inicia sesión con un proveedor real, dos veces.** Añade una registration `google` con nada más que `client_id` y `client_secret` y confirma que la página de login gana un botón; luego añade una segunda registration sobre `keycloak` con solo `provider.corp.issuer_uri` puesto, y observa cómo el descubrimiento rellena cuatro endpoints que nunca escribiste. Deja el emisor sin servicio y recarga `/login`: un botón desaparece con un aviso en el log, el otro sigue funcionando.
