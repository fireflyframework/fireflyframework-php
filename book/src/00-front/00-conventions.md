...
A `source:` excerpt may **cut** whole lines it does not need — an import list, a long docblock, the middle of a method. Every cut is marked with a line that is exactly `// …` (or `# …` in a file, such as YAML, whose comments start with a hash), and the comparison resumes after what it has already matched, so what remains is still, in order, what the file says. Three things a cut may never do, all refused by the guard: swallow the declaration it belongs to — leaving a `{` with nothing above it saying *what* is being declared — hollow a method out to an empty body, which would print a method that appears to do nothing, or leave a docblock standing on its own. A docblock is a claim *about* a declaration, so an excerpt that shows one shows its `/**`, its closing line, and at least one line of the class, method or constant it describes; starting partway down a docblock would print a paragraph of commentary about code the page never shows.
