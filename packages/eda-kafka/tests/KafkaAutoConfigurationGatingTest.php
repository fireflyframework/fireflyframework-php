 ...
 * AND the EventPublisher bean must NOT be registered/active — unless firefly.eda.provider=kafka is the active
 * provider.
 *
 * KAFKA-SPECIFIC SUBTLETY (unlike RabbitMQ, whose connection factory only connects lazily inside health()):
 * KafkaAutoConfiguration::eventPublisher() THROWS a clear RuntimeException at bean-RESOLUTION time when
 * provider=kafka and ext-rdkafka is absent — the WANTED fail-fast (a clear boot error, never a silent no-op).
 * On this machine (no ext-rdkafka), EagerSingletonsPass (BootPhase::EagerSingletons, 900 — resolves every
 * non-#[Lazy] Singleton bean, #[Order]-sorted, straight from the manifest) reaches that bean during
 * `$app->boot()`, and the throw surfaces there, aborting the boot pipeline entirely. So this file proves two
 * DISTINCT, genuine things — not one vacuous "it didn't crash" check:
 *
 ...
 *       kernel the failed boot ran against then proves EventPublisher/KafkaHealthIndicator are GENUINELY
 *       registered under provider=kafka — not merely inferred from the throw alone.
 *   (B) INERTNESS: booting under provider=memory (or provider absent) completes with NO throw, and neither
 *       KafkaHealthIndicator nor EventPublisher is bound in the resulting ApplicationContext — proving the
 *       Kafka beans are fully gated OFF for a non-kafka provider.
 *
...
it('(B) does not register KafkaHealthIndicator or EventPublisher when provider = memory', function () {
...
it('(B) does not register KafkaHealthIndicator or EventPublisher when firefly.eda.provider is absent', function () {
