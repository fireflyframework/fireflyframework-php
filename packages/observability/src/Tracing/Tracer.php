 ...
 *
 * startSpan() starts a span AND makes it the current one until end(): a span started with no explicit parent
 * is a child of the current span; an invalid parent (SpanContext::invalid()) starts a new root; a valid remote
 * parent (from W3CTraceContextPropagator::extract()) continues that trace. currentSpan() is what a log
 * processor reads. trace() is the M12 convenience kept intact: start, run, record a throwable as an ERROR
 * status plus an exception event, rethrow, end — the callback now receives the span, and a zero-argument
