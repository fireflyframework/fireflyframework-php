 ...
 * `half-open-max-calls`, `minimum-number-of-calls`, `record-on`) already lives in configuration, where an
 * operator can widen a window at 3am without a deploy. That is Resilience4j's own split, and it is the
 * reason this attribute wraps the programmatic component rather than reimplementing a policy: there is
 * exactly one CircuitBreaker in this package, its state lives in the shared ResilienceStore, and both call
 * paths trip the SAME breaker — an attribute on a method and a hand-written `$registry->circuitBreaker()`
 * beside it are not two breakers that happen to share a name.
 *
