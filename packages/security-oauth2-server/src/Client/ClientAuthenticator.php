 ...
 *
 * Two methods in one request are `invalid_request` (RFC 6749 §2.3: MUST NOT); everything else that fails is
...
final class ClientAuthenticator
{
    ...
    /** `private_key_jwt`: the client must list the method and hold a JWK set, and the assertion must verify against it. */
    ...
    /**
     * Every method the request presents, in a fixed order: Basic, a JWT assertion, a posted secret, a bare id.
     *
    ...
    public static function presented(Request $request): array
    {
        ...
        $assertionType = $request->input('client_assertion_type');
    ...
    private function verifyAssertion(string $assertion, RegisteredClient $client): void
    {
        try {
            ...
            $claims = (array) JWT::decode($assertion, JWK::parseKeySet($client->clientSettings->jwkSet ?? ['keys' => []]));
        } catch (Throwable $e) {
            $this->logger?->info("private_key_jwt assertion of client [{$client->clientId}] did not verify: ".$e::class);
        ...
        if (($claims['iss'] ?? null) !== $client->clientId || ($claims['sub'] ?? null) !== $client->clientId || ! $audienceOk || ! isset($claims['exp'])) {
